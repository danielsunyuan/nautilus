#!/usr/bin/env python3
"""Stream top-of-book for the Polymarket World Cup Winner event."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Stream World Cup winner top-of-book from the CLOB.")
    ...
    parser.add_argument("--max-teams", type=int, default=15)
...
async def _run(args: argparse.Namespace) -> int:
    ...
    max_teams = len(entries) if args.max_teams <= 0 else min(args.max_teams, len(entries))
    first = True
    reset = False
    line_count = 1
    ...
    while True:
        ...
        rendered = render_winner_stream(title=title, rows=rows, style=style)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.max_teams < 0:
        parser.error("--max-teams must be >= 0")
    try:
