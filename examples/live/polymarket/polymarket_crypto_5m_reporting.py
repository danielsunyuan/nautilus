...
def _rounds_in_rows(rows: list[dict[str, Any]]) -> int:
    ...
    session_ids = {
        str(row["session_id"])
        for row in rows
        if row.get("event") == "strategy_result" and row.get("session_id")
    }
...
def build_summary(*, report_root: str | Path, now: datetime | None = None) -> dict[str, Any]:
    ...
    for strategy in strategies.values():
        ...
        total_stake = float(strategy.pop("total_stake"))
    ...
    total_stake = sum(
        (float(row["net_pnl"]) / float(row["roi"])) if row["roi"] not in (None, 0) else 0.0
